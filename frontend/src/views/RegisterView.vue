<template>
  <div class="auth-page">
    <div class="auth-card">
      ...
      <p class="auth-sub">Pievienojies Latvijas mūzikas kopienai!</p>
 
      <div v-if="error" class="alert-error">{{ error }}</div>
 
      <div class="field">
        <label>Lietotājvārds</label>
        <input v-model="form.lietotajvards" type="text" placeholder="tavs_vards" />
      </div>
      <div class="field">
        <label>E-pasts</label>
        <input v-model="form.e_pasts" type="email" placeholder="tavs@epasts.lv" />
      </div>
      <div class="field">
        <label>Parole</label>
        <input v-model="form.parole" type="password" placeholder="Min. 6 simboli" />
      </div>
      ...
      <p class="auth-switch">
        Jau ir konts? <router-link to="/login">Ienākt</router-link>
      </p>
    </div>
  </div>
</template>
...
const form      = ref({ lietotajvards: '', e_pasts: '', parole: '' })
const error     = ref('')
...
async function submit() {
  ...
  try {
    ...
    else error.value = e.response?.data?.message || 'Kļūda reģistrējoties.'
  } finally {
...
.auth-logo { font-size: 40px; color: var(--accent); text-align: center; margin-bottom: 16px; }
.auth-title {
